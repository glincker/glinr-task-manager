...
export function modelsCommands(): Command {
  ...
  cmd
    ...
    .action(async (options: { provider?: string; json?: boolean }) => {
      ...
      try {
        ...
        const result = await api.get<ModelsResponse>(`/api/providers/models${params}`);
        spin.stop();
        ...
        if (defaultModel) console.log(chalk.dim(`Default: ${defaultModel}`));
      } catch (err) {
        spin.stop();
  ...
  cmd
    ...
    .action(async (model: string, options: { json?: boolean }) => {
      ...
      try {
        const result = await api.get<ModelInfo>(`/api/providers/models/${encodeURIComponent(model)}`);
        spin.stop();
        if (!result.ok) { error(result.error || `Model ${model} not found`); process.exit(1); }
        const m = result.data!;
  ...
  cmd
    ...
    .action(async (model: string, options: { json?: boolean }) => {
      ...
      try {
        const result = await api.post<{ default: string }>('/api/providers/models/default', { model });
        spin.stop();
  ...
  cmd
    ...
    .action(async (options: { json?: boolean }) => {
      ...
      try {
        const result = await api.get<ModelAliasMap>('/api/providers/models/aliases');
        spin.stop();
        ...
        if (entries.length === 0) { info('No model aliases configured.'); return; }
  ...
  cmd
    ...
    .action(async (model: string, options: { json?: boolean }) => {
      ...
      try {
        const result = await api.post<TestResult>(`/api/providers/models/${encodeURIComponent(model)}/test`);
        spin.stop();
        if (!result.ok) { error(result.error || `Failed to test ${model}`); process.exit(1); }
