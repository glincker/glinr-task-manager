...
describe('security utilities', () => {
  ...
  describe('isSafeRedirectUrl', () => {
    ...
    it('handles paths that look like URLs but are safe relative paths', () => {
      // 'not-a-url' is treated as a relative path by URL parser, so it's safe (same-origin)
      // Only truly invalid syntax would fail (but most strings become valid relative paths)
      expect(isSafeRedirectUrl('/valid/path', 'not a valid url')).toBe(false);
    });
  });
