...
const PAGE_CONTEXTS: Record<string, PageContext> = {
  dashboard: {
    ...
    quickActions: [
      { label: 'Status report', prompt: 'Give me a status report of my workspace. What are the key metrics and any items needing attention?' },
      { label: "What's new?", prompt: "What's happened recently in my workspace? Any updates I should know about?" },
      { label: 'Productivity tips', prompt: 'Based on my current workload, what productivity tips do you have?' },
    ],
  },
  tickets: {
    type: 'tickets',
    ...
    quickActions: [
      { label: 'Create ticket', prompt: 'Help me create a new ticket. Ask me what it should be about.' },
      { label: 'Find tickets', prompt: 'Help me find tickets. What would you like to search for?' },
      { label: 'Ticket overview', prompt: 'Give me an overview of my tickets - how many are open, in progress, and done?' },
    ],
  },
  ticketDetail: {
    type: 'ticket-detail',
    ...
    quickActions: [
      { label: 'Summarize', prompt: 'Summarize this ticket and its current status for me.' },
      { label: 'Suggest next steps', prompt: 'Based on this ticket, what should be the next steps to resolve it?' },
      { label: 'Draft comment', prompt: 'Help me draft a status update comment for this ticket.' },
    ],
  },
  projects: {
    type: 'projects',
    ...
    quickActions: [
      { label: 'Create project', prompt: 'Help me create a new project. What should it be called and what prefix should it use?' },
      { label: 'Project status', prompt: 'Give me a status update on all my projects.' },
      { label: 'Sprint planning', prompt: 'Help me plan a new sprint. What tickets should be included?' },
    ],
  },
  projectDetail: {
    type: 'project-detail',
    ...
    quickActions: [
      { label: 'Project summary', prompt: 'Give me a summary of this project - progress, blockers, and upcoming work.' },
      { label: 'Sprint status', prompt: "What's the status of the current sprint in this project?" },
      { label: 'Create ticket', prompt: 'Help me create a new ticket for this project.' },
    ],
  },
  tasks: {
    type: 'tasks',
    ...
    quickActions: [
      { label: 'Task status', prompt: 'Give me an overview of my task queue - pending, running, and completed.' },
      { label: 'Failed tasks', prompt: 'Are there any failed tasks that need attention?' },
      { label: 'Agent activity', prompt: 'What are my AI agents currently working on?' },
    ],
  },
  summaries: {
    type: 'summaries',
    ...
    quickActions: [
      { label: 'Recent work', prompt: 'What work has been completed recently? Show me recent summaries.' },
      { label: 'Find summary', prompt: 'Help me find a summary about a specific topic or task.' },
    ],
  },
  costs: {
    type: 'costs',
    ...
    quickActions: [
      { label: 'Cost summary', prompt: "What's my AI usage and cost this month?" },
      { label: 'Compare models', prompt: 'Compare the costs between different AI models I use.' },
    ],
  },
  gateway: {
    type: 'gateway',
    ...
    quickActions: [
      { label: 'Run workflow', prompt: 'Help me run an AI workflow. What task should I execute?' },
      { label: 'Gateway status', prompt: "What's the status of the AI gateway?" },
    ],
  },
  agents: {
    type: 'agents',
  ...
  webhooks: {
    ...
    quickActions: [
      { label: 'Webhook status', prompt: 'Show me the status of all webhooks.' },
      { label: 'Setup webhook', prompt: 'Help me set up a new webhook integration.' },
    ],
  },
  failed: {
    type: 'failed',
    ...
    quickActions: [
      { label: 'Analyze failures', prompt: 'Analyze the failed tasks and tell me what went wrong.' },
  ...
  memory: {
    ...
    quickActions: [
      { label: 'Search memory', prompt: 'Search my memory for information about...' },
  ...
  cron: {
    ...
    quickActions: [
      { label: 'Job status', prompt: 'Show me the status of all my scheduled jobs. How many are active, paused, or failed?' },
      { label: 'Recent failures', prompt: 'Are there any failed scheduled jobs? Show me the recent failures and what went wrong.' },
      { label: 'Create job', prompt: 'Help me create a new scheduled job. What task should it automate and how often should it run?' },
      { label: 'Job history', prompt: 'Show me the execution history of my scheduled jobs. Which ones ran recently?' },
    ],
  },
  default: {
    type: 'default',
...
function usePageContext(): { context: PageContext; entityId?: string } {
  ...
  return useMemo(() => {
    ...
    if (projectMatch) {
      return { context: PAGE_CONTEXTS.projectDetail, entityId: projectMatch[1] };
    }
    ...
    if (path.startsWith('/summaries')) return { context: PAGE_CONTEXTS.summaries };
    ...
    if (path.startsWith('/settings')) return { context: PAGE_CONTEXTS.settings };
...
function ToolCallDisplay({ toolCall }: { toolCall: NonNullable<ChatMessage['toolCalls']>[0] }) {
...
export function FloatingChatbot() {
  ...
  const configuredProviderTypes = new Set(providers.map((p: { type: string }) => p.type));

  const availableModels = aliases
    .filter((alias: { provider: string }) => configuredProviderTypes.has(alias.provider))
    .reduce((acc: Record<string, { alias: string; provider: string; model: string }>, alias: { alias: string; provider: string; model: string }) => {
      const key = alias.provider;
      if (!acc[key] || alias.alias.length < acc[key].alias.length) {
        acc[key] = alias;
      }
      return acc;
    }, {} as Record<string, { alias: string; provider: string; model: string }>);

  const sortedModels = Object.values(availableModels)
    .sort((a, b) => {
      const aHealthy = providers.find((p: { type: string; healthy: boolean }) => p.type === a.provider)?.healthy ? 1 : 0;
      const bHealthy = providers.find((p: { type: string; healthy: boolean }) => p.type === b.provider)?.healthy ? 1 : 0;
  ...
  useEffect(() => {
    if (!selectedModel && sortedModels.length > 0) {
      const firstHealthy = sortedModels.find(m =>
        providers.find((p: { type: string; healthy: boolean }) => p.type === m.provider)?.healthy
      );
      if (firstHealthy) {
  ...
  const sendMessageMutation = useMutation({
    mutationFn: async (userMessage: string) => {
      ...
      const response = await api.chat.conversations.sendMessageWithTools(currentConversationId!, {
        content: userMessage,
        enableTools: true,
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className={cn(
            ...
            'rounded-3xl',
            'shadow-[0_8px_40px_rgba(0,0,0,0.12),0_2px_12px_rgba(0,0,0,0.08),inset_0_1px_0_rgba(255,255,255,0.4)]',
            'dark:shadow-[0_8px_40px_rgba(0,0,0,0.5),0_2px_12px_rgba(0,0,0,0.3),inset_0_1px_0_rgba(255,255,255,0.05)]',
            // Responsive sizes - larger on desktop
            isExpanded
              ? 'w-[calc(100vw-2rem)] sm:w-[480px] md:w-[560px] lg:w-[640px] h-[70vh] sm:h-[600px] md:h-[700px]'
              : 'w-[calc(100vw-2rem)] sm:w-[400px] md:w-[440px] h-[60vh] sm:h-[520px] md:h-[580px]'
          )}
        >
          {/* Header - Glass effect */}
          <div className="flex items-center justify-between px-4 py-3 border-b border-white/20 dark:border-white/5 bg-white/30 dark:bg-white/5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground">
                  ...
                  {entityId && ticketData && (
                    <span className="text-primary">• GLINR-{(ticketData as { sequence?: number }).sequence}</span>
                  )}
                  {entityId && projectData && (
                    <span className="text-primary">• {(projectData as { key?: string }).key}</span>
                  )}
                </div>
              </div>
            </div>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button className="flex items-center gap-1.5 px-2 py-1 rounded-lg text-[10px] font-medium bg-muted/40 hover:bg-muted/60 border border-border/30 transition-colors mr-2">
                    <span className={cn(
                      ...
                      providers.find((p: { type: string; healthy: boolean }) => p.type === aliases.find((a: { alias: string }) => a.alias === selectedModel)?.provider)?.healthy
                        ? 'bg-green-500'
                        : 'bg-amber-400'
                    )} />
                ...
                <DropdownMenuContent align="end" className="w-44 glass-dropdown">
                  ...
                  {sortedModels.map((alias) => {
                    const provider = providers.find((p: { type: string }) => p.type === alias.provider);
                    const isHealthy = provider?.healthy;
                    const isSelected = (selectedModel || currentModel) === alias.alias;
          ...
          {/* Messages */}
          <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-none liquid-fade-y">
            {isLoadingHistory ? (
              ...
              <div className="flex flex-col items-center justify-center h-full text-center px-4">
                ...
                <div className="mt-4 pt-4 border-t border-border w-full">
                  <p className="text-[10px] text-muted-foreground uppercase tracking-wide mb-2">I can help you</p>
          ...
          {/* Input - Glass effect */}
          <div className="p-3 border-t border-white/20 dark:border-white/5 bg-white/30 dark:bg-white/5">
            <div className="flex items-end gap-2">
              <textarea
                ...
                className="flex-1 px-3 py-2 text-sm bg-white/50 dark:bg-black/30 border border-white/30 dark:border-white/10 rounded-xl resize-none focus:outline-none focus:ring-2 focus:ring-primary/50 min-h-[40px] max-h-[120px] placeholder:text-muted-foreground/60 disabled:opacity-50"
                style={{
                  height: 'auto',
              ...
              <Button
                onClick={handleSend}
                disabled={!input.trim() || sendMessageMutation.isPending || isLoadingHistory}
                size="sm"
      ...
      <button
        ...
        className={cn(
          'fixed bottom-6 right-6 z-50 h-14 w-14 rounded-full flex items-center justify-center transition-all duration-300 hover:scale-110',
          // Glass effect for button
          'backdrop-blur-xl backdrop-saturate-150',
          'shadow-[0_4px_24px_rgba(0,0,0,0.15),inset_0_1px_0_rgba(255,255,255,0.3)]',
          'dark:shadow-[0_4px_24px_rgba(0,0,0,0.4),inset_0_1px_0_rgba(255,255,255,0.1)]',
          isOpen
            ? 'bg-white/60 dark:bg-zinc-800/70 text-muted-foreground border border-white/30 dark:border-white/10'
            : 'bg-primary/90 text-primary-foreground border border-primary/50'
        )}
        title={isOpen ? 'Close chat' : `Open GLINR Assistant (${pageContext.name})`}
      >
        {isOpen ? (
